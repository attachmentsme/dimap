  ...
  var _this = this,
    server = http.createServer(function(req, res) {
      var msgId = req.url.split('/')[1],
        filename = req.url.split('/')[2] ? decodeURIComponent( req.url.split('/')[2] ) : undefined;
...
Proxy.prototype.proxyToAttachment = function(res, msgId, filename, originalUrl) {
  ...
  this.onConnection(originalUrl, this.attachmentGrabberSettings, function(settings) {
    ...
    // if we don't have a valid filename and msgID
    // don't even try to connect to IMAP.
    ...
    if (filename && msgId) {
      ...
      console.log('could not parse msgId and filename from url:', originalUrl)
      res.writeHead(404);
      res.end();
    }
